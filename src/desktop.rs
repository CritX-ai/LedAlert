//! Passive desktop metadata collection. No desktop service names are acquired here.
...
impl DesktopMonitor {
    /// Starts one worker thread; all D-Bus discovery and authentication happen off the GUI thread.
    pub fn start() -> anyhow::Result<Self> {
        ...
        // Never block the daemon or GUI. Queue loss advances the generation so persistent lights
        // cannot survive a dropped close, even when there is no room to enqueue a reset event.
                    ...
                    Ok(runtime) => {
                        runtime.block_on(async {
                            tokio::select! {
                                ...
                                _ = async {
                                    tokio::join!(
                                        notification_loop(&worker_shared, &notification_tx),
                                        session_loop(&worker_shared),
                                    );
                                } => {},
                            }
                        });
    ...
    pub fn snapshot(&self) -> DesktopState {
        ...
        if shared
            .media_checked
            .is_some_and(|checked| checked.elapsed() > MEDIA_FRESHNESS)
        {
...
impl Drop for DesktopMonitor {
    fn drop(&mut self) {
        ...
        // Cancellation drops in-flight RPCs and streams. Never wait indefinitely for D-Bus or
        // Tokio's blocking pool (which can be used internally by address resolution).
